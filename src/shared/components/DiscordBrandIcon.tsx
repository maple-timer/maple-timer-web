...
type DiscordBrandIconProps = Omit<SVGProps<SVGSVGElement>, "height" | "viewBox" | "width"> & {
  size?: number;
  tone?: "blurple" | "currentColor" | "white";
};

export function DiscordBrandIcon({
  className,
  size = 16,
  style,
  tone = "blurple",
  ...props
}: DiscordBrandIconProps) {
  const classes = ["discord-brand-icon", className].filter(Boolean).join(" ");
