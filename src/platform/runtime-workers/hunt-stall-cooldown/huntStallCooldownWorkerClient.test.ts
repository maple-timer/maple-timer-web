import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import {
  createHuntStallCooldownWorkerClient,
  type HuntStallCooldownWorkerResult,
} from "./huntStallCooldownWorkerClient";
import type {
  HuntStallCooldownWorkerRequest,
  HuntStallCooldownWorkerResponse,
} from "./huntStallCooldownWorkerTypes";

const runtimeWorkerHealthMocks = vi.hoisted(() => ({
  createRuntimeWorkerFailure: vi.fn(
    ({ error, fallbackMessage }: { error?: unknown; fallbackMessage: string }) =>
      error instanceof Error
        ? error
        : new Error(typeof error === "string" && error ? error : fallbackMessage),
  ),
  markRuntimeWorkerReady: vi.fn(),
}));

vi.mock("../runtimeWorkerHealth", () => runtimeWorkerHealthMocks);

class FakeWorker {
  onmessage: ((event: MessageEvent<HuntStallCooldownWorkerResponse>) => void) | null = null;
  onerror: ((event: ErrorEvent) => void) | null = null;
  postMessage = vi.fn<
    (request: HuntStallCooldownWorkerRequest, transfer?: Transferable[]) => void
  >();
  terminate = vi.fn();

  respond(response: HuntStallCooldownWorkerResponse) {
    this.onmessage?.({ data: response } as MessageEvent<HuntStallCooldownWorkerResponse>);
  }
}

let workers: FakeWorker[];

describe("huntStallCooldownWorkerClient", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.clearAllMocks();
    workers = [];
    vi.stubGlobal(
      "Worker",
      class {
        constructor() {
          const worker = new FakeWorker();
          workers.push(worker);
          return worker;
        }
      },
    );
  });

  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });

  it("transfers the crop and resolves the matching worker response", async () => {
    const client = createHuntStallCooldownWorkerClient();
    const imageData = createImageData();
    const pending = client.process(imageData);
    const request = workers[0].postMessage.mock.calls[0][0];

    expect(request).toMatchObject({
      type: "process",
      id: 1,
      imageData,
    });
    expect(workers[0].postMessage.mock.calls[0][1]).toEqual([
      imageData.data.buffer,
    ]);

    const response = createWorkerResult();
    workers[0].respond({
      type: "processed",
      id: request.id,
      ...response,
    });

    await expect(pending).resolves.toEqual(response);
    expect(runtimeWorkerHealthMocks.markRuntimeWorkerReady).toHaveBeenCalledWith(
      "hunt-stall-cooldown",
    );
  });

  it("terminates the worker and reports a failure after the fixed timeout", async () => {
    const client = createHuntStallCooldownWorkerClient();
    const pending = client.process(createImageData()).then(
      () => null,
      (error: unknown) => error,
    );

    await vi.advanceTimersByTimeAsync(999);
    expect(workers[0].terminate).not.toHaveBeenCalled();
    await vi.advanceTimersByTimeAsync(1);

    const error = await pending;
    expect(error).toBeInstanceOf(Error);
    expect((error as Error).message).toBe("hunt-stall-cooldown-worker-timeout");
    expect(workers[0].terminate).toHaveBeenCalledOnce();
    expect(runtimeWorkerHealthMocks.createRuntimeWorkerFailure).toHaveBeenCalledWith(
      expect.objectContaining({
        feature: "hunt-stall-cooldown",
        code: "worker-timeout",
      }),
    );
  });

  it("rejects pending work when reset releases the worker", async () => {
    const client = createHuntStallCooldownWorkerClient();
    const pending = client.process(createImageData()).then(
      () => null,
      (error: unknown) => error,
    );

    client.reset();

    const error = await pending;
    expect(error).toBeInstanceOf(Error);
    expect((error as Error).message).toBe("hunt-stall-cooldown-worker-reset");
    expect(workers[0].terminate).toHaveBeenCalledOnce();
  });
});

function createImageData(): ImageData {
  return new ImageData(new Uint8ClampedArray(4 * 4 * 4), 4, 4);
}

function createWorkerResult(): HuntStallCooldownWorkerResult {
  return {
    result: {
      value: 7,
      confidence: 0.92,
      debug: {
        recognizedText: "7",
      },
    },
    activity: {
      fingerprint: "0".repeat(256),
      gridColumns: 16,
      gridRows: 16,
      foregroundRatio: 0.25,
    },
    performance: {
      recognitionMs: 2.5,
      totalMs: 3,
    },
  };
}
